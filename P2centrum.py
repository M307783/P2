...
def samplingmask(ky_size, acceleration_factor=90, center_fraction=0.20, seed=42):
    """
    ...
    chosen = np.random.choice(outside_center, size=int(n_center), replace=True)
    mask[mask] = True
...
stats_text = (
    f"Rækker beholdt: {n_kept} / {ky_size}\n"
    f"Procent beholdt: {n_percentile:.2f}%\n"
    f"MSE:  {mse:.1f}\n"
    f"RMSE: {rmse:.4f}"
)
...
fig, axes = plt.subplots(2,3,figsize=(14,6)) # ændre ncols ud fra hvor mange rekonstruktioner vi vil vise
